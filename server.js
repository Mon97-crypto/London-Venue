...
const DRY_RUN = process.env.DRY_RUN === '1' || !RESEND_API_KEY;
...
function entryFor(data, id) {
  data[id] ??= { status: 'not_contacted', history: [] };
  return data[id];
}
...
app.get('/api/venues', async (_req, res, next) => {
  try {
    const [venues, outreach] = await Promise.all([loadVenues(), loadOutreach()]);
    res.json(
      venues.map((v) => ({
        ...v,
...
// Photos: when a venue has no image URLs in venues.json, read them from its own
// website (og:image plus large <img> tags) and cache the result on disk.
const PHOTOS_FILE = path.join(__dirname, 'data', 'photos-cache.json');
let photoCache = existsSync(PHOTOS_FILE) ? JSON.parse(await readFile(PHOTOS_FILE, 'utf8')) : {};
...
async function photosFor(venue) {
  ...
  const cached = photoCache[venue.id];
  ...
  if (!inflight.has(venue.id)) {
    inflight.set(
      ...
      scrapePhotos(venue.website)
        .then(async (photos) => {
          photoCache[venue.id] = { at: Date.now(), photos };
          await writeFile(PHOTOS_FILE, JSON.stringify(photoCache, null, 2));
          return photos;
        })
        .catch((err) => {
...
app.get('/api/photos/:id', async (req, res, next) => {
  try {
    const venue = (await loadVenues()).find((v) => v.id === req.params.id);
    if (!venue) return res.status(404).json({ error: 'Unknown venue' });
    res.json({ photos: await photosFor(venue) });
  } catch (err) {
    next(err);
  }
});
...
app.post('/api/send', async (req, res, next) => {
  try {
    ...
    const outreach = await updateOutreach((data) => {
      const entry = entryFor(data, venue.id);
...
app.post('/api/outreach/:id', async (req, res, next) => {
  try {
    ...
    const venues = await loadVenues();
    if (!venues.some((v) => v.id === req.params.id)) return res.status(404).json({ error: 'Unknown venue' });
    const outreach = await updateOutreach((data) => {
      const entry = entryFor(data, req.params.id);
      const at = new Date().toISOString();
...
app.listen(PORT, () => {
  console.log(`London Venue RFP app on http://localhost:${PORT}${DRY_RUN ? '  (DRY RUN – emails are not sent)' : ''}`);
});
